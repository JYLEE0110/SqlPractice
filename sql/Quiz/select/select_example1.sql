...
--1. 덧셈연산자를 이용하여 모든 사원에 대해서 $300의 급여 인상을 계산한 후 사원의 이름, 급여, 
--  인상된 급여를 출력하시오.

select ename, sal, sal+300 as IncSal from emp order by 3 desc; -- order by 속성 순서로도 가능

-- 2. 사원의 이름, 급여, 연간 총 수입을 총 수입이 많은 것부터 작은 순으로 출력하시오, 
-- 연간 총수입은 월급에 12를 곱한 후 $100의 상여금을 더해서 계산하시오.

select ename, sal, (sal*12+nvl(comm,0)) as annual_sal from emp order by annual_sal desc;
...
select ename, sal, deptno from emp where sal between 2000 and 3000 and deptno not in(10) order by ename;
