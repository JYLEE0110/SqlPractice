...
select ename, substr(to_char(hiredate,'yyyy-mm-dd'),1,7)as hire_year_month from emp;

--17. SUBSTR 함수를 사용하여 4월에 입사한 사원을 출력하시오.
select ename, hiredate from emp where substr(to_char(hiredate,'mm'),1) = '04';

--18. MOD 함수를 사용하여 사원번호가 짝수인 사람만 출력하시오.
select ename, empno from emp where mod(empno,2) =0;

--19. 입사일을 년도는 2자리(YY), 월은 숫자(MM)로 표시하고 요일은 약어 (DY)로 지정하여 출력하시오.
select ename, to_char(hiredate,'yy-mm-dy') as hireday from emp;

--20. 올해 몇 칠이 지났는지 출력하시오. 현재날짜에서 올해 1월 1일을 뺀 결과를 출력하고 
--TO_DATE 함수를 사용하여데이터 형을 일치 시키시오.
select trunc(sysdate-to_date('2023/01/01','yyyy/mm/dd')) as day from dual;
...
select ename, sal, nvl(DECODE(job,
'ANALIST',sal+200,
'SALESMAN',sal+180,
'MANAGER',sal+150,
'CLERK',sal+100),sal) as IncSal
from emp;

--23. 모든 사원의 급여 최고액, 최저액, 총액 및 평균 급여를 출력하시오.
-- 평균에 대해서는 정수로 반올림하시오.
select 
to_char(max(sal)*12,'L999,999,999')as max,
to_char(min(sal)*12,'L999,999,999')as min,
to_char(sum(sal)*12,'L999,999,999')as sum,
to_char(round(avg(sal))*12,'L999,999,999')as avg
from emp;

--24. 각 담당 업무 유형별로 급여 최고액, 최저액, 총액 및 평균 액을 출력하시오. 
--평균에 대해서는 정수로 반올림 하시오.

select job, max(sal) as max, min(sal) as min, round(avg(sal))as avg from emp group by job;
...
select deptno, count(*), round(avg(sal),2) as avg_of_dept from emp group by deptno;
...
20, 'DALLAS',
30, 'CHICAGO',
40, 'BOSTON')as loc, count(*) as total_emp, round(avg(sal)) as avg_of_sal from emp group by deptno;